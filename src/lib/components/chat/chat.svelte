...
<div class="fixed right-4 bottom-4 w-80">
  <Card class="shadow-lg">
    ...
    <ScrollArea class="h-96 p-4">
      {#each messages as message}
        <div class="mb-4 {message.sender === 'user' ? 'text-right' : 'text-left'}">
          <div class="inline-block bg-{message.sender === 'user' ? 'primary' : 'secondary'} text-{message.sender === 'user' ? 'primary' : 'secondary'}-foreground rounded-lg px-4 py-2 max-w-[80%]">
            {message.text}
          </div>
        </div>
      {/each}
    ...
    <form on:submit|preventDefault={sendMessage} class="p-4 border-t">
      <div class="flex gap-2">
        <Input bind:value={newMessage} placeholder="Type a message..." class="flex-grow" />
